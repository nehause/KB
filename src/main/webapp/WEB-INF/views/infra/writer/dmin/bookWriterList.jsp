		...
		<!-- header navBar end -->
			<div class="container-fluid">
				<div class="row" style="padding-left: 20px; padding-right: 20px;">
					...
					<!-- sideBar end -->
					<div class="col-lg-10">
						<form method="post" id="BWLForm" name="BWLForm" role="search">
							<input type="hidden" id="mainKey" name="mainKey" value="">
							<input type="hidden" id="book_writerSeq" name="book_writerSeq" value="">
								...
								<div class="row" style="margin-bottom: 10px;">
									<div class="col-lg-2">
										<select class="form-select bg-light" id="searchOption" name="searchOption" aria-label="selectsearch">
											...
											<option value="1" <c:if test="${vo.searchOption eq 1}">selected</c:if>>작가 저서 번호</option>
											<option value="2" <c:if test="${vo.searchOption eq 2}">selected</c:if>>책 번호</option>
											<option value="3" <c:if test="${vo.searchOption eq 3}">selected</c:if>>책 이름</option>
											<option value="4" <c:if test="${vo.searchOption eq 4}">selected</c:if>>작가 번호</option>
											<option value="5" <c:if test="${vo.searchOption eq 5}">selected</c:if>>작가 이름</option>
										</select>
									</div>
									<div class="col-lg-2">
										...
										<thead>
											<tr style="background-color: black">
												<td class="checkboxsize"> 
													<input class="form-check-input" type="checkbox" name="allBookWriterCheck" value="allBookWriterCheck" onclick="selectAll(this);">
												</td>
												<th>#</th>
												<th>번호</th>
												<th>책 번호</th>
											...
											<c:choose>
												...
												<c:otherwise>
													<c:forEach items="${list}" var="list" varStatus="status">
														<tr onclick="location.href='javascript:goView(<c:out value="${list.book_writerSeq }"/>)'" style="cursor: pointer;">
															<td>
																<input class="form-check-input" type="checkbox" name="bookWriterCheck" value="bookWriterCheck" onclick="checkSelectAll();">
															</td>
															<td><c:out value="${status.count }"/></td>
															<td><c:out value="${list.book_writerSeq }"/></td>
															<td><c:out value="${list.book_bookSeq }"/></td>
															<td><c:out value="${list.name }"/></td>
															<td><c:out value="${list.writer_writerSeq }"/></td>
...
<!-- modal end -->
	...
	function checkSelectAll()  {
		...
		const checkboxes = document.querySelectorAll('input[name="bookWriterCheck"]');

		const checked = document.querySelectorAll('input[name="bookWriterCheck"]:checked');

		const selectAll = document.querySelector('input[name="allBookWriterCheck"]');
	...
	function selectAll(selectall)  {
		const checkboxes = document.getElementsByName('bookWriterCheck');
